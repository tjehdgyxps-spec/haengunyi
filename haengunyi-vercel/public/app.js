...
function resolveSymbol(query) {
  ...
  // Default: treat as Korean search term, try partial match or pass through
...
stockInput.addEventListener('input', () => {
  ...
  const matches = searchStocks(q);
  if (matches.length === 0) {
    // If looks like US ticker, show that option
    if (isUSTicker(q) && q.length >= 1) {
      suggestionsDiv.innerHTML = `<div class="suggestion-item" data-symbol="${q.toUpperCase()}">
